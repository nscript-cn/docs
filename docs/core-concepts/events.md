...
事件是由事件发生器发出的用于表示一种特定操作的消息。这种操作可能由用户操作（比如一次触摸）或程序逻辑（比如从服务器下载的图片已经下载完毕）产生。产生事件的对象称为**事件发送器（event sender）**（简称为**发送器（sender）**）或是**事件发生器（event raiser）**。销毁该事件的对象称为**事件监听器（event listener）**（简称为**监听器（listener）**）或是**事件处理器（event handler）**。
